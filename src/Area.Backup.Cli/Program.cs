...
public static class Program
{
    ...
    private static void PrintUsage()
    {
        Console.WriteLine("""
Usage:
  Area.Backup.Cli backup --config <path-to-json>
  Area.Backup.Cli validate --backup <path> [--mode Quick|Full] [--test-restore]
  Area.Backup.Cli restore --backup <path> --destination <dir> [--source <id>] [--filter <prefix>]
  Area.Backup.Cli info --backup <path>
  Area.Backup.Cli list --repository <dir>
  Area.Backup.Cli benchmark [--files <count>] [--size <kb>]

Examples:
  Area.Backup.Cli backup --config ./config.sample.json
  Area.Backup.Cli validate --backup C:\Backup\Atual\2026\09\20260902-143000-full.backup --mode Full
  Area.Backup.Cli restore --backup C:\Backup\Atual\2026\09\20260902-143000-full.backup --destination C:\Restore
  Area.Backup.Cli benchmark --files 5000 --size 32
""");
    }
